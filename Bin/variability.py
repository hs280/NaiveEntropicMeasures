...
def periodic_autocorrelation(signal,outfile,key):
    ...
    plt.grid(True)
    if outfile !=None:
        plt.savefig(f'{outfile}/autocorrelation_{key}', dpi=1200)
...
def plot_bar_chart_with_error_bars(dict1, dict2, labels=['dict1', 'dict2'], ylabel='Noise Proportion',
    ...
    # Show plot
    plt.tight_layout()
    plt.savefig(f'{outfolder}/bar_chart{y_label_stripped}.png', dpi=1200)
...
def plot_entropy(entropy,save_folder=None):
    ...
    if save_folder != None:
        plt.savefig(os.path.join(save_folder, 'entropy.png'), dpi=1200)
    else:
        plt.show()
...
def plot_heatmap(matrix, x_label='Predictor Location', y_label='Target Location', name='theils_u_heatmap.png',Label = '', save_folder=None):
    ...
    if save_folder is not None:
        plt.savefig(os.path.join(save_folder, name), dpi=1200)
    else:
        plt.show()
    ...
    if save_folder is not None:
        ...
        plt.savefig(os.path.join(save_folder, sum_plot_name), dpi=1200)
    else:
        plt.show()
...
def plot_residue_probability_heatmap(df, column_name='amino_acid_sequence', save_folder=None):
    ...
    if save_folder is None:
        ...
        plt.savefig(os.path.join(save_folder, 'residue_probability_heatmap.png'), dpi=1200)
...
#         plt.show()
#     else:
#         plt.savefig(os.path.join(save_folder, dpi=1200), dpi=600)
...
def plot_normalized_cumsum(data,save_folder = None,name='residue_cumsum'):
    """
